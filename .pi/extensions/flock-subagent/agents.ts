...
export function discoverAgents(cwd: string, scope: AgentScope): AgentDiscoveryResult {
	...
	const bundledAgents = loadAgentsFromDir(bundledAgentsDir, "package");
	const userAgents = scope === "project" ? [] : loadAgentsFromDir(userDir, "user");
	const projectAgents = scope === "user" || !projectAgentsDir ? [] : loadAgentsFromDir(projectAgentsDir, "project");

	const agentMap = new Map<string, AgentConfig>();

	// Package agents are the Flock defaults. User agents can override package
	// defaults, and project agents can override both when project scope is enabled.
