...
- review rework dispatch requires the `Review rework` approval category and goes through the section 5b rework pass only — one prompt to the existing Herdr worker, never a new agent, branch, or PR
...
1. project config contains a `Rework` policy explicitly allowing one rework pass, and the `Review rework` approval category permits it
2. every blocking finding is severity Important or Minor — any Critical finding stops the run with `review blocking` for a human, no rework
...
| "A blocking review can be bounced straight back to the worker." | Rework requires the explicit config Rework policy and Review rework approval: one pass per issue per run, non-Critical findings only, the existing worker session, and a fresh gate run plus fresh review afterward. |
