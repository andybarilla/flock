...
Flock is a cross-platform, open-source local development environment manager — a Go + Wails desktop app serving as a community alternative to Laravel Herd. It manages local vhosts, SSL, PHP runtimes, and database services via a plugin architecture that can be extended to support any language stack.
...
- **Site registry** — persists known local sites to `~/.config/flock/sites.json`
- **System tray + Wails GUI**

### First-party plugins (bundled, loaded at startup)
- `flock-php` — PHP binary version management, PHP-FPM pool lifecycle, per-site version selection
- `flock-databases` — MySQL, PostgreSQL, Redis service lifecycle
- `flock-ssl` — mkcert integration for local CA trust and per-site/wildcard TLS certs

### Future plugins (same API, not bundled in v1)
- `flock-node`, `flock-quarkus`, and others — external authors use the same plugin interface
...
**Site entry** (`~/.config/flock/sites.json`):
```json
{
...
Sites are added via the GUI or a `flock link` CLI subcommand (same binary).
...
**Adding a site:**
1. User picks a directory in the GUI (or runs `flock link`)
2. Core infers domain from directory name (`myapp` → `myapp.test`); user can override
3. `flock-ssl` issues a cert for the domain
...
**PHP-FPM pool management**: One pool per PHP version in use. Pools start on demand (first site needing that version) and stay running until Flock quits.

**Startup sequence:**
1. Load config + site registry
2. Init plugins in registration order
3. `flock-ssl` ensures CA is trusted and certs exist for all sites
4. `flock-php` starts FPM pools for all PHP versions referenced by current sites
...
- Plugin `Init`/`Start` failures are non-fatal — Flock starts with the plugin marked degraded and shows a GUI warning. A broken database plugin does not affect PHP sites.
...
- All errors logged to `~/.local/share/flock/flock.log` (XDG on Linux/macOS; `AppData\Roaming\flock` on Windows).
